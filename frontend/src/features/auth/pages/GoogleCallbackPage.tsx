...
export default function GoogleCallbackPage() {
  ...
  useEffect(() => {
    ...
    if (success === 'true' && accessToken && refreshToken) {
      // Store tokens (user info will be fetched by the auth store)
      setAuth(
        { id: '', email: '', name: '' }, // Placeholder, will be updated by profile fetch
        accessToken,
        refreshToken
      );
      ...
      if (window.opener) {
        window.opener.postMessage(
          { type: 'GOOGLE_OAUTH_SUCCESS', accessToken, refreshToken },
